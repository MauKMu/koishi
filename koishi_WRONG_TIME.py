...
class InterpolatedTrajectory:
  ...
  def get_next_position(self, step):
    # add step * node's time to self.time
    increment = step * self.nodes[self.index][2] 
    # if new self.time >= 1.0, move on to next index, jumping over
    # multiple indices if needed, and make self.time < 1.0
    # also consider different speeds in each node
    # TODO: fix how time works. It has the opposite
    # effect now -- shorter time values make the
    # trajectory "slower" at those nodes.
    # Should be easy: just replace 1.0 with self.nodes[self.index][2] below, and remove multiplying by that same value
    if self.time + increment >= 1.0:
      remaining_step = step - (1.0 - self.time) / self.nodes[self.index][2]
      increment = remaining_step * self.nodes[self.next_index][2]
      # Update indices
      self.index = self.next_index
      self.next_index = self._get_next_index()

      while increment >= 1.0:
        # Find portion of step that contributed to movement and
        # subtract it from remaining step
        remaining_step -= (1.0) / self.nodes[self.index][2]
        increment = remaining_step * self.nodes[self.next_index][2]
        #print("inc:", str(increment))
        #print("stp:", str(remaining_step))
        if increment < 0.0:
          increment = 0.0
        # Update indices
        self.index = self.next_index
    ...
    self.last_pos = self.curr_pos
    self.curr_pos = (start_x + delta_x * self.time, 
                     start_y + delta_y * self.time)
...
def main():
  ...
  dwg = svgwrite.drawing.Drawing('genetics.svg')
